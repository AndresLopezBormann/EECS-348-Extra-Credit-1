...
void PreferenceAlgo(int allPreferences[10][5])
{
    ...
    int DepartmentChosen[5] = {0};                      // If the Department has chosen
    int DeptAssignment[5] = {-1, -1, -1, -1, -1};    // What the Department has chosen

    for (int i = 0; i < 5; ++i) { // Row
        for (int j = 0; j < 5; ++j) { // Column
            if (DepartmentChosen[j] == 0) { // Department has not chosen
                if (studentsChosen[allPreferences[i][j] - 1] == 0) {     // student is Available
                        ...
                        DepartmentChosen[j] = 1;                         // Department  changes to chosen
                    ...
                    for (int k = 5; k < 10; ++k){
                        if ((studentAssignment[allPreferences[i][j] - 1] + 1) == allPreferences[k][allPreferences[i][j] - 1]){ // if Previous index == Programmer Choice
                            ...
                            DepartmentChosen[studentAssignment[allPreferences[i][j] - 1]] = 0; // Reset old department to unchosen
                            DepartmentChosen[j] = 1;                                           // Set new department to chosen
